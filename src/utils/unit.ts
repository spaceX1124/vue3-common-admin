/**
 * @Author: tudou@Date: 2025-04-02 14:29:36
 * @Description: 金额转换
 * @parameter: xx
 */
...
export function useAmountConverter () {
  ...
  // 响应式转换方法
  const createConverter = (fromUnit: UnitType, toUnit: UnitType, options?: ConvertOptions) => {
    const input = ref<string | number>('')
    const output = computed(() => convert(input.value, fromUnit, toUnit, options))

    return { input, output }
  }

  return { convert, createConverter }
}
