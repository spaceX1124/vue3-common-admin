...
export interface ISchema {
    ...
    formItemClass?: string; // 表单项的栅格布局, 如 'col-span-2 col-start-2'， 占满2列空间 从第二列开始 相当于前面空了一列
    ...
    async?: { // 需要异步请求数据
        ...
        hiddenOptions?: string | ((data: Record<string, any>) => boolean); // 需要隐藏的项，确定要隐藏的项的id，可传入字符串（多个逗号分隔），不确定可通过函数处理
        disabledOptions?: string | ((data: Record<string, any>) => boolean); // 需要置灰的项，确定要置灰的项的id，可传入字符串（多个逗号分隔），不确定可通过函数处理
    };
    isEcho?: boolean; // 表格回显，如后端返id，需要匹配然后回显就传true，直接展示后端返回不传或设为false即可
    cellRenderer?: (obj: any) => any // 自定义渲染内容，如jsx语法等，可拿到最新的表头数据，有那种异步获取数据注入到表头的
}
